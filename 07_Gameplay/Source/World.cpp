...
// As soon as an enemy position enters the battlefield, the corresponding enemy is created and inserted to the scene graph, and the spawn point is removed. 
// The World class member function getBattlefieldBounds(), returns sf::FloatRect to the battlefield area, similar to getViewBounds(). 
// The battlefield area extends the view area by a small rectangle at the top, inside which new enemies spawn before they enter the view. 
// If an enemys y coordinate lies below the battlefield's top member, the enemy will be created at its spawn point. 
...
// Our actual dispatch function is now rather simple. 
// We check the whole scene graph for collisions, and fill the set with collision pairs. 
// Then, we iterate through the set and differentiate between the collisions categories.

// https://github.com/SFML/SFML-Game-Development-Book/blob/master/07_Gameplay/Source/World.cpp
