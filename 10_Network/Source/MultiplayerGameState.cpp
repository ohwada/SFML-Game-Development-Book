...
// we went created a new state MultiplayerGameState, which is very similar to GameState
// you see, the three typical state-related methods to draw, update, and handleEvent
// Several variables are new related to the management of different playersand socket connections:
...
//Because both modes are almost equal, it wouldnt even make sense to have another state. 
// The only difference is thatif the state is the host, it will launch a background server in its constructor andshut it down again in its destructor! 


// https://github.com/SFML/SFML-Game-Development-Book/blob/master/10_Network/Source/MultiplayerGameState.cpp
