...
// Now, inside GameServer::executionThread() is where all the magic happens.
// First rule is that while executionThread() doesnt return a value, the parallelthread is alive. 
// There are three main logical steps in the server thread: initialization,the server loop, and termination

// to tell the sf::TcpListener socket to start accepting new connections. 
// This iswhat allows the players to join our server. 



// https://github.com/SFML/SFML-Game-Development-Book/blob/master/10_Network/Source/GameServer.cpp
...
void GameServer::executionThread()
{
	...
	sf::Time stepInterval = sf::seconds(1.f / 60.f); // 17 msec
	sf::Time stepTime = sf::Time::Zero;
	sf::Time tickInterval = sf::seconds(1.f / 20.f); // 50 msec
